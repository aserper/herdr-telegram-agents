...
- [ ] **Short done posts**: `Skip short done posts` → `30s`; a one-word prompt answered in 5 s → no done post, log `reason=short_turn`; a longer task posts as usual; set it back to `Off`
  ...
  - [ ] an unsupported agent, or a Claude/Pi pane with no usable transcript, still posts the screen and the log shows `reply source unavailable`
...
- [ ] **`/reply`**: a Claude Code and a Pi topic each post the complete latest answer with headings, lists and code formatted; a reply longer than five Telegram parts is not cut; an unsupported agent answers with `⚠️ no complete transcript reply is available`; `/reply` in General says agent commands live in the agent's topic
